...
def cert_fair_attack(x, y, x_control, weight, number, fair_constraint, bar):
    ...
    for i in range(int(number)):
        ...
        for yy in y_list:
            for aa in a_list:
                center = torch.mean(x[(y == yy) & (a == aa)], dim=0)
    ...
    all_x = all_x + np.random.normal(0, 0.02, all_x.shape)
